...
typedef struct {
    ...
    /* Destination node (0xFFFF = broadcast) */
    uint64_t timestamp;
    /* Message timestamp */
    uint32_t payload_size;
    /* Size of payload following header */
    uint32_t flags;
    /* Message flags */
...
typedef struct {
    ...
    uint8_t cache_state;
    /* Current cache state */
    uint8_t new_cache_state;
    /* New cache state after operation */
    uint16_t sharers_bitmap;
    /* Bitmap of sharing nodes */
    uint32_t version;
    /* Cache line version */
    uint8_t data[64];
    /* Cacheline data */
...
typedef struct {
    ...
    /* Base address of this node's memory */
    uint64_t memory_size;
    /* Size of memory managed by this node */
...
typedef struct {
    ...
    /* Current owner node */
    uint16_t sharers_bitmap;
    /* Nodes with shared copies */
    uint8_t current_state;
    /* Current MESI state */
    uint8_t requested_state;
    /* Requested MESI state */
    uint32_t version;
    /* Cacheline version */
    uint8_t data[24];
...
typedef struct {
    ...
    /* Global cache state */
    uint8_t home_node;
    /* Home node for this address */
    uint8_t owner_node;
    /* Current owner (for E/M states) */
    uint8_t flags;
    /* Entry flags */
    uint16_t sharers_bitmap;
    /* Bitmap of nodes with copies */
    uint16_t pending_bitmap;
    /* Nodes with pending operations */
    uint32_t version;
    /* Version number */
...
struct TCPCalibrationResult {
    ...
    // Measured latency (ns)
    double o_s;
    // Measured send overhead (ns)
    double o_r;
    // Measured receive overhead (ns)
    double g;
    // Measured gap - 1/bandwidth (ns)
    uint64_t samples; // Number of samples taken
    bool valid;
    // Whether calibration succeeded

    TCPCalibrationResult() : L(0), o_s(0), o_r(0), g(0), samples(0), valid(false) {}
};

/* Per-node TCP connection state */
struct TCPNodeConnection {
    ...
    bool connected;
    // Connection state
    TCPCalibrationResult calibration;
    // Per-node calibration data

    TCPNodeConnection() : remote_addr(0), remote_buffer_size(0), connected(false) {}
};
...
struct DistDirectoryEntry {
    ...
    DistDirectoryEntry()
        : cacheline_addr(0), state(DIST_CACHE_INVALID), home_node(0), owner_node(UINT32_MAX), version(0),
          last_access_time(0) {}
    ...
    DistDirectoryEntry(DistDirectoryEntry &&other) noexcept
        : cacheline_addr(other.cacheline_addr), state(other.state), home_node(other.home_node),
          owner_node(other.owner_node), sharers(std::move(other.sharers)), version(other.version),
...
struct DistNodeInfo {
    ...
    DistNodeInfo()
        : node_id(UINT32_MAX), state(NODE_STATE_UNKNOWN), memory_base(0), memory_size(0), last_heartbeat(0),
          pending_requests(0), total_requests(0), total_latency_ns(0) {}

    // Copy constructor
    DistNodeInfo(const DistNodeInfo &other)
        : node_id(other.node_id), hostname(other.hostname), state(other.state), memory_base(other.memory_base),
          memory_size(other.memory_size), last_heartbeat(other.last_heartbeat),
          pending_requests(other.pending_requests), total_requests(other.total_requests),
...
public:
    ...
    bool send_message_wait_response(uint32_t dst_node, const dist_message_t &req, dist_message_t &resp,
                                    int timeout_ms = 5000);
...
/* TCP Transport Layer for Distributed Server */
...
private:
    ...
    std::map<uint32_t, TCPNodeConnection> connections_;
    ...
    std::map<uint32_t, TCPCalibrationResult> calibration_results_;
    std::mutex calibration_mutex_;

public:
    DistributedTCPTransport(uint32_t node_id, const std::string &bind_addr, uint16_t port);
    ~DistributedTCPTransport();
    ...
    // TCP read/write (emulate remote data access via request/response)
    bool tcp_read(uint32_t dst_node, uint64_t remote_offset, void *local_buf, size_t size);
    bool tcp_write(uint32_t dst_node, uint64_t remote_offset, const void *local_buf, size_t size);

    // LogP calibration via TCP ping-pong
    TCPCalibrationResult calibrate_node(uint32_t dst_node, uint32_t num_samples = 1000);
    TCPCalibrationResult get_calibration(uint32_t node_id) const;
    TCPCalibrationResult get_aggregate_calibration() const;
...
public:
    ...
    /* TCP transport management */
    bool connect_tcp_node(uint32_t node_id, const std::string &addr, uint16_t port);
...
private:
    ...
    /* Forwarding */
    int forward_read(uint32_t target_node, uint64_t addr, void *data, size_t size, uint64_t *latency_ns);
    int forward_write(uint32_t target_node, uint64_t addr, const void *data, size_t size, uint64_t *latency_ns);

    /* TCP-based forwarding (used when transport_mode_ is TCP or HYBRID) */
    int forward_read_tcp(uint32_t target_node, uint64_t addr, void *data, size_t size, uint64_t *latency_ns);
    int forward_write_tcp(uint32_t target_node, uint64_t addr, const void *data, size_t size, uint64_t *latency_ns);

    /* RDMA-based forwarding (used when transport_mode_ is RDMA or HYBRID) */
    int forward_read_rdma(uint32_t target_node, uint64_t addr, void *data, size_t size, uint64_t *latency_ns);
    int forward_write_rdma(uint32_t target_node, uint64_t addr, const void *data, size_t size, uint64_t *latency_ns);
